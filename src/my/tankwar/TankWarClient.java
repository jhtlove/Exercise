...
//			try
//			{
//				//getClass拿到这个类编译后的class；config根目录需要放到classpath
//				p.load(this.getClass().getResourceAsStream("config" + File.separator + "tank.properties"));//空指针错误
//			}
//			catch (IOException e)
//			{
...
//				e.printStackTrace();
//			}
			tankCount = Integer.parseInt(PropertiesMgr.p.getProperty("tankCount"));//拿到是字符串，转换一下
			level ++;
		}
		else
		{
			...
			{
				//getClass拿到这个类编译后的class；config根目录需要放到classpath
				p.load(this.getClass().getResourceAsStream("config/tank.properties"));
			}
			catch (IOException e)
			{
				// TODO Auto-generated catch block
			...
			tankCount = Integer.parseInt(p.getProperty("reProduceTanks"));
			level = 1;
		}
		tanks.clear();
		missiles.clear();
